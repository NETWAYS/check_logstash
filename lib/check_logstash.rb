...
#
# Changelog:
#   - 0.7.4 fix reload error detection, add verbose option
...
class CheckLogstash
  ...
  module CLI
    ...
    def parse(check, args)
      ...
      OptionParser.new do |opts|
        ...
        opts.on('-v', '--verbose', 'Run verbosely') { |v| check.verbose = v }
	  ...
	  puts '[DEBUG] ' + check.inspect if check.verbose
    end
  end # module CLI
  ...
  def config_reload_health(result)
    ...
    if Gem::Version.new(result.get('version')) >= Gem::Version.new('6.0.0')
      ...
      for named_pipeline in result.get('pipelines') do
        if named_pipeline[0] != ".monitoring-logstash"
            config_reload_errors = result.get('pipelines.' + named_pipeline[0] + '.reloads.failures').to_i
			config_reload_error_message = result.get('pipelines.' + named_pipeline[0] + '.reloads.last_error.message').to_s.strip
            puts '[DEBUG] pipelines.' + named_pipeline[0] + '.reloads.failures=' + config_reload_errors.to_s if self.verbose
            puts '[DEBUG] config_reload_error_message='+config_reload_error_message.inspect if self.verbose
			...
			puts '[DEBUG] last_success_timestamp='+config_reload_last_success_timestamp.inspect if self.verbose
			puts '[DEBUG] last_failure_timestamp='+config_reload_last_failure_timestamp.inspect if self.verbose
			if config_reload_last_success_timestamp < config_reload_last_failure_timestamp
				if config_reload_errors > 0
					...
					config_reload_errors_report = config_reload_errors_report + " " + named_pipeline[0] + ": " + config_reload_error_message + ";"
				else
					config_reload_errors_report = config_reload_errors_report + " " + named_pipeline[0] + ": " + "OK;"
				end
			else
				config_reload_errors_report = config_reload_errors_report + " " + named_pipeline[0] + ": " + "OK;"
			end
      ...
      puts '[DEBUG] config_reload_errors='+config_reload_errors.inspect if check.verbose
      puts '[DEBUG] config_reload_error_message='+config_reload_error_message.inspect if check.verbose
